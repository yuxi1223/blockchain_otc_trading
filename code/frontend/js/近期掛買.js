...
$(document).ready(async function () {
  ...
  var buyArray = new Array();                                   //所有買單array
  var buy = await contract_SearchBuy(IFNFT_contract, stock);    //取得買單
...
async function createContract(address) {
  ...
  //       success: res => { // 成功的話執行...
  //         alert("merchandisePost成功");
  //       },
  //       error: err => { // 失敗的話執行...
  //         alert("merchandisePost失敗");
  //       },
  //     })
  //   })
    ...
    //                 // contentType: "application/json",
    //                 success: res => { // 成功的話執行...
    //                     alert("stockholderPost成功");
    //                 },
    //                 error: err => { // 失敗的話執行...
    //                     alert("stockholderPost失敗");
    //                 },
    //             })
    //         }
    //     )
...
function createBuyTable(buyArray) {
  ...
  for (var i = 0; i < buyArray.length; i++) {
    table.append(
      $('<tr>').append(
        $('<td>').text(new Date().toLocaleString()).addClass('tt'),
        $('<td>').text(buyArray[i].address.substring(0, 5)).addClass('tt'),
        $('<td>').text(buyArray[i].quantity).addClass('td'),
        $('<td>').text(buyArray[i].price).addClass('td'),
        $('<td>').html("<button class='btn sell' id='" + buyArray[i].address + "'>賣出</button>").addClass('td')
      ));
  ...
  $("button").click(async function () {
    ...
    var order = "buy";
    await contract_BuyByIndex(IFNFT_contract, stock, address, quantity, price);
 
    // alert("賣出給：" + address + "股數：" + quantity + "價格：" + price);
