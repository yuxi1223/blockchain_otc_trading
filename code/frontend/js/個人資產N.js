...
function createInvestTable(investCompanyArray) {
  ...
  var arrheader = ['公司名稱', '統一編號', '持有股數',"占比", '掛買單股數','','掛賣單股數',''];
  ...
  for (var i = 0; i < investCompanyArray.length; i++) {
    table.append(
      $('<tr>').append(
        $('<td>').html("<a href='./公司N.html' id='" + investCompanyArray[i].symbol + "'>"+investCompanyArray[i].name+"</a>").addClass('td'),
        ...
        $('<td>').html("<button class='btnbuy' id='" + investCompanyArray[i].address + "'>取消</button>").addClass('td'),
        $('<td>').text(investCompanyArray[i].totalSell).addClass('td'),
        $('<td>').html("<button class='btnsell' id='" + investCompanyArray[i].address + "'>取消</button>").addClass('td')
      ));
  }
  var div = $('<div>').addClass('div');
  div.append(table)
  $(".main-area").prepend(div,$('<div>').addClass('control').html("<img src='../img/控制向.jpg' style='height: 30px'>"));
  $("a").click( function(){
  ...
  $(".btnbuy").click(function(){
    var stock = $(this).parent().prev().prev().prev().prev().text();
    // alert(stock);
     localStorage.setItem('queryStock', stock)
    localStorage.setItem('assets', "buy");
    $(location).attr('href', './個人資產掛買賣單N.html');
  })
  $(".btnsell").click(function(){
    var stock = $(this).parent().prev().prev().prev().prev().prev().prev().text();
     localStorage.setItem('queryStock', stock)
    localStorage.setItem('assets', "sell");
    $(location).attr('href', './個人資產掛買賣單N.html');
  })
}
